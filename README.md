...
```
You are an advanced AI coding assistant and I want you to build for me a really cool space travel booking app like a futuristic Expedia from Dubai to different space places with AI and dashboards and charts and stuff. It should have everything a modern web app has, including user login and authentication and AI recommendations and beautiful UI and responsive design and also it has to be extremely fast and secure and scalable and easy to maintain and also work on mobile and desktop and maybe even as a PWA in the future so keep that in mind.

The idea is that users can search and book trips to various space destinations like different space stations, moon hotels, Mars colonies, deep space cruises etc. There should be multiple classes like economy, business, luxury, VIP zero gravity and the user should be able to filter and sort all of these and compare prices and pick the best option. Also show some kind of charts and visualizations about prices and availability over time with AI and machine learning predicting the best time to travel and what package to choose. Also show countdown timers for launches and some kind of health/advice tips about going to space and what to pack, preferably generated by AI.

Use Python for the backend and whatever is best for the frontend (React or not, you decide) and deploy it somewhere easy like Replit or maybe something else if you think it’s better, but keep it simple at the same time make sure it can scale to a lot of users. Try to make the code modular, clean and well-documented with comments everywhere and also include tests where appropriate, but don’t spend too much time on tests, focus on features. Also make sure you add some animations (maybe CSS, maybe JavaScript) to make it feel dynamic, like some rockets flying or orbits moving or something like that.

The AI part should be really smart: it should look at user preferences and history and budgets and somehow figure out what’s the best destination and date and cabin and show that as a recommendation, maybe like “Recommended for you” cards. Also have some sort of pricing prediction like airlines do, so let’s say “Prices likely to go up” or “Prices likely to drop” based on historical data (you can fake the data if needed). It should also handle edge cases and errors nicely with friendly messages so the user is not confused, and maybe log errors somehow but don’t overcomplicate that.

I want you to generate all the code necessary to run this as a full app including configuration, main files, HTML, CSS, JS, backend, whatever is needed so that I can just copy and paste it and run it. Don’t ask me too many questions, just make reasonable assumptions. Try to finish everything in one go and make sure it’s production-ready or close to it because I might use this as a demo for something important, but also don’t over-engineer because this is kind of a prototype. Just balance it."
```
**Problems:**
- Vague requirements
